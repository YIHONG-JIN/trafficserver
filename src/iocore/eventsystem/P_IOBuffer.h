...
//
//////////////////////////////////////////////////////////////////
TS_INLINE char *PipeIOBufferReader::start() {
  throw std::runtime_error("Not applicable for PipeIOBufferReader");
}

TS_INLINE char *PipeIOBufferReader::end() {
  throw std::runtime_error("Not applicable for PipeIOBufferReader");
}

TS_INLINE int64_t PipeIOBufferReader::read_avail() {
  return static_cast<PipeIOBuffer*>(mbuf)->data_in_pipe;
}

TS_INLINE bool PipeIOBufferReader::is_read_avail_more_than(int64_t size) {
  return read_avail() > size;
}

TS_INLINE int PipeIOBufferReader::block_count() {
  return 1;
}

TS_INLINE int64_t PipeIOBufferReader::block_read_avail() {
  return read_avail();
}

TS_INLINE std::string_view PipeIOBufferReader::block_read_view() {
  throw std::runtime_error("Not applicable for PipeIOBufferReader");
}

TS_INLINE void PipeIOBufferReader::skip_empty_blocks() {
  // No-op for PipeIOBufferReader
}

TS_INLINE void PipeIOBufferReader::clear() {
  // call base class clear
  IOBufferReader::clear();
}

TS_INLINE void PipeIOBufferReader::reset() {
  // call base class reset
  IOBufferReader::reset();
}

TS_INLINE void PipeIOBufferReader::consume(int64_t n) {
...
TS_INLINE IOBufferReader *PipeIOBufferReader::clone() {
  throw std::runtime_error("Cloning not supported for PipeIOBufferReader");
}

TS_INLINE void PipeIOBufferReader::dealloc() {
  static_cast<PipeIOBuffer*>(mbuf)->dealloc_reader(this);
}

TS_INLINE IOBufferBlock *PipeIOBufferReader::get_current_block() {
  throw std::runtime_error("Not applicable for PipeIOBufferReader");
}

TS_INLINE bool PipeIOBufferReader::current_low_water() {
  return static_cast<PipeIOBuffer*>(mbuf)->current_low_water();
}

TS_INLINE bool PipeIOBufferReader::low_water() {
  return static_cast<PipeIOBuffer*>(mbuf)->low_water();
}

TS_INLINE bool PipeIOBufferReader::high_water() {
  return static_cast<PipeIOBuffer*>(mbuf)->high_water();
}

TS_INLINE int64_t PipeIOBufferReader::memchr([[maybe_unused]] char c , [[maybe_unused]] int64_t len, [[maybe_unused]] int64_t offset) {
  throw std::runtime_error("Not supported in PipeIOBufferReader");
}

// Read data from the pipe into the buffer.
TS_INLINE int64_t PipeIOBufferReader::read(void *buf, int64_t len) {
  auto *pipe_buf = static_cast<PipeIOBuffer*>(mbuf);
...
TS_INLINE char *PipeIOBufferReader::memcpy([[maybe_unused]] void *buf, [[maybe_unused]] int64_t len, [[maybe_unused]] int64_t offset) {
  throw std::runtime_error("Not supported in PipeIOBufferReader");
}

TS_INLINE char &PipeIOBufferReader::operator[]([[maybe_unused]] int64_t i) {
  throw std::runtime_error("Not supported in PipeIOBufferReader");
}
...
//
//////////////////////////////////////////////////////////////////
TS_INLINE PipeIOBuffer::PipeIOBuffer() : fd{-1, -1}, reader_allocated(false), data_in_pipe(0), pipe_capacity(0) {
...
TS_INLINE void PipeIOBuffer::fill(int64_t len) {
...
TS_INLINE void PipeIOBuffer::consume(int64_t len) {
  if (len > data_in_pipe) {
...
TS_INLINE void PipeIOBuffer::append_block([[maybe_unused]] IOBufferBlock *b) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE void PipeIOBuffer::append_block([[maybe_unused]] int64_t asize_index) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE void PipeIOBuffer::add_block() {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE void PipeIOBuffer::append_xmalloced([[maybe_unused]] void *b, [[maybe_unused]] int64_t len) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE void PipeIOBuffer::append_fast_allocated([[maybe_unused]] void *b, [[maybe_unused]] int64_t len, [[maybe_unused]] int64_t fast_size_index) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}


TS_INLINE int64_t PipeIOBuffer::write(const void *buf, int64_t nbytes) {
...
TS_INLINE int64_t PipeIOBuffer::write([[maybe_unused]] IOBufferReader *r, [[maybe_unused]] int64_t len, [[maybe_unused]] int64_t offset) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE int64_t PipeIOBuffer::write([[maybe_unused]] IOBufferChain const *chain, [[maybe_unused]] int64_t len, [[maybe_unused]] int64_t offset) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE IOBufferBlock *PipeIOBuffer::first_write_block() {
  throw std::runtime_error("Not applicable for PipeIOBuffer");
}

TS_INLINE char *PipeIOBuffer::buf() {
  throw std::runtime_error("Not applicable for PipeIOBuffer");
}

TS_INLINE char *PipeIOBuffer::buf_end() {
  throw std::runtime_error("Not applicable for PipeIOBuffer");
}

TS_INLINE char *PipeIOBuffer::start() {
  throw std::runtime_error("Not applicable for PipeIOBuffer");
}

TS_INLINE char *PipeIOBuffer::end() {
  throw std::runtime_error("Not applicable for PipeIOBuffer");
}

TS_INLINE int64_t PipeIOBuffer::block_write_avail() {
  return write_avail();
}

TS_INLINE int64_t PipeIOBuffer::current_write_avail() {
  return write_avail();
}

TS_INLINE int64_t PipeIOBuffer::write_avail() {
  return pipe_capacity - data_in_pipe;
}

TS_INLINE int64_t PipeIOBuffer::block_size() {
  return pipe_capacity;
}

TS_INLINE bool PipeIOBuffer::high_water() {
  return is_max_read_avail_more_than(this->water_mark);
}

TS_INLINE bool PipeIOBuffer::low_water() {
  return write_avail() <= water_mark;
}

TS_INLINE bool PipeIOBuffer::current_low_water() { return low_water(); }

// Allocate a reader for the PipeIOBuffer with the given MIOBufferAccessor
TS_INLINE IOBufferReader *PipeIOBuffer::alloc_accessor(MIOBufferAccessor *anAccessor) {
  if (reader_allocated) {
...
TS_INLINE IOBufferReader *PipeIOBuffer::alloc_reader() {
  if (reader_allocated) {
    throw std::runtime_error("PipeIOBuffer supports only a single reader");
  }
...
TS_INLINE IOBufferReader *PipeIOBuffer::clone_reader([[maybe_unused]] IOBufferReader *r) {
  // PipeIOBuffer supports only a single reader, so cloning is not supported, but return the existing reader
  return &pipe_reader;
}

TS_INLINE void PipeIOBuffer::dealloc_reader(IOBufferReader *e) {
  if (&pipe_reader == e) {
...
TS_INLINE void PipeIOBuffer::set([[maybe_unused]] void *b, [[maybe_unused]] int64_t len) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}
...
TS_INLINE int64_t PipeIOBuffer::write([[maybe_unused]] IOBufferBlock const *b, [[maybe_unused]] int64_t len, [[maybe_unused]] int64_t offset) {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE int64_t PipeIOBuffer::max_read_avail() {
  return data_in_pipe;
}

TS_INLINE bool PipeIOBuffer::is_max_read_avail_more_than(int64_t size) {
  return data_in_pipe > size;
}

// PipeIOBuffer has only one block, which is the pipe itself, so block count is always 1
TS_INLINE int PipeIOBuffer::max_block_count() {
  return 1;
}

TS_INLINE void PipeIOBuffer::check_add_block() {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

TS_INLINE void PipeIOBuffer::reset() {
...
TS_INLINE void PipeIOBuffer::init_readers() {
  throw std::runtime_error("Not supported in PipeIOBuffer");
}

// Release the external resources associated with the PipeIOBuffer
TS_INLINE void PipeIOBuffer::dealloc() {
...
TS_INLINE void PipeIOBuffer::clear() {
  data_in_pipe = 0;
  pipe_capacity = 0;
  dealloc();
}
